...
frCumCatch <- frTS %>% 
  group_by(catchReg, Region1Name, month) %>%
  summarize(meanPropC = mean(propCatch, na.rm = T)) %>% 
  group_by(catchReg, Region1Name) %>% 
  mutate(cumCatch = cumsum(meanPropC))

ggplot(frCumCatch %>% filter(catchReg == "NWVI")) +
  geom_line(aes(x = month, y = cumCatch, colour = Region1Name)) +
